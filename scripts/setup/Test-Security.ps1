...
foreach ($composeFile in $ComposeFiles) { $composeArgs += @("--file", $composeFile) }
...
$enc = docker compose @composeArgs exec -T `
...
if ($enc -ne "scram-sha-256") { throw "Expected scram-sha-256, got: $enc" }
...
foreach ($pair in @(
    ...
    if ($LASTEXITCODE -ne 0) { throw "SCRAM auth failed for $($pair.User): $result" }
...
try {
    $wrongResult = docker compose @composeArgs exec -T `
        -e "PGPASSWORD=deliberately_wrong_password_xyz" -e "PGCONNECT_TIMEOUT=3" pgpool `
...
if ($wrongExitCode -eq 0) { throw "Expected connection with wrong password to fail, but it succeeded" }
...
$trustCount = docker compose @composeArgs exec -T pg-primary `
...
if ($trustCount -ne "0") { throw "pg_hba.conf contains $trustCount trust entry/entries - must be zero" }
...
foreach ($node in @("pg-primary", "pg-standby-a", "pg-standby-dr")) {
    ...
    $ErrorActionPreference = "Continue"
    try {
        $portOutput = docker compose @composeArgs port $node 5432 2>&1
        $exitCode = $LASTEXITCODE
    }
    finally {
        $ErrorActionPreference = $previousErrorActionPreference
    }
    $portOutput = (($portOutput | ForEach-Object { $_.ToString() }) -join "`n").Trim()
    Write-Host "[security] $node port 5432 -> '$portOutput' (exit $exitCode)"
    if ($exitCode -eq 0 -and $portOutput -notin @("", ":0")) { throw "$node port 5432 is published on host: $portOutput" }
}

$previousErrorActionPreference = $ErrorActionPreference
$ErrorActionPreference = "Continue"
try {
    $pcpOutput = docker compose @composeArgs port pgpool 9898 2>&1
    $pcpExit = $LASTEXITCODE
}
finally {
    $ErrorActionPreference = $previousErrorActionPreference
}
$pcpOutput = (($pcpOutput | ForEach-Object { $_.ToString() }) -join "`n").Trim()
Write-Host "[security] pgpool port 9898 (PCP) -> '$pcpOutput' (exit $pcpExit)"
if ($pcpExit -eq 0 -and $pcpOutput -notin @("", ":0")) { throw "PCP port 9898 is published on host: $pcpOutput" }
